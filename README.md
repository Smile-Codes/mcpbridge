...
Open the chat window via **Radius → MCP → Chat** or press **F12**.
...
1. **Open Unity** → wait for compile → **Radius → MCP → Chat** (or F12). Window opens = installed.
...
3. **(Optional) Node bridge** — to let an external Claude Code CLI drive Unity:
   ...
   - In Unity: **Radius → MCP → Server** must be 🟢 ON (auto-starts on Unity open).

## Notes

- **Allow Write Commands** (Radius → MCP → Allow Write Commands) must be ON for any command that
